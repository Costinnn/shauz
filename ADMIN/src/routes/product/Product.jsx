...
const Product = () => {
  ...
  const handleDeleteProduct = async () => {
    const productUrl =
      import.meta.env.VITE_DELETE_PRODUCT_LOCAL_URL + product._id;
  ...
  const handleDeleteArrayItem = async (field, stringToDelete) => {
    const productUrl =
      import.meta.env.VITE_DELETE_ARRAYITEM_LOCAL_URL + product._id;

    try {
      const response = await axios.patch(productUrl, { field, stringToDelete });
  ...
  const handleAddArrayItem = async (field, stringToAdd, position = 0) => {
    const productUrl =
      import.meta.env.VITE_ADD_ARRAYITEM_LOCAL_URL + product._id;

    try {
      const response = await axios.patch(productUrl, {
        field,
        stringToAdd,
        position,
      });
  ...
  const handleUpdateValue = async (field, newValue) => {
    const productUrl =
      import.meta.env.VITE_UPDATE_VALUE_LOCAL_URL + product._id;

    try {
      const response = await axios.patch(productUrl, { field, newValue });
  ...
  const handleUpdateSize = async (sizeToChange, newValue) => {
    const productUrl = import.meta.env.VITE_UPDATE_SIZE_LOCAL_URL + product._id;
      ...
      <div className="update-photos">
        ...
        <div className="photo-input">
          ...
          <button
            className="button1"
            onClick={() =>
              newImg ? handleAddArrayItem("images", newImg, newImgPosition) : ""
            }
          >
            Add
          </button>
